...
public class HttpUtils {
    ...
    private Object aomenHttps;
    private Object sportHttps;
    private Object switchHttps;
     ...
     * BASEURL
     */
 //   private final static String BASE_URL = "http://222.186.151.162/yxsm_manage/";
    private final static String BASE_URL = "http://www.qltb360.com/yxsm_manage/";
    private final static String BASE_URL2 = "http://www.qltb360.com:8888/";
    private final static String SWITCH_URL ="http://vipapp.01appddd.com/Lottery_server/";
//    private final static String SPORT_URL = "http://op.juhe.cn/onebox/football/";
    private final static String SPORT_URL = "http://v.juhe.cn/toutiao/";
    /**
     * aomen
     */
    public static final String AO_MEN = "http://1114600.com:8080/";
    /**
     * 分页数据，每页的数量
     */
    ...
    public <T> T getYunServer(Class<T> a) {
        if (loginHttps == null) {
            synchronized (HttpUtils.class) {
                if (loginHttps == null) {
                    loginHttps = getBuilder(BASE_URL).build().create(a);
                }
            }
        }
        return (T) loginHttps;
    ...
    public <T> T getSwitchServer(Class<T> a) {
        if (switchHttps == null) {
            synchronized (HttpUtils.class) {
                if (switchHttps == null) {
                    switchHttps = getBuilder(SWITCH_URL).build().create(a);
                }
            }
        }
        return (T) switchHttps;
    }


    public <T> T getAoMenServer(Class<T> a) {
        if (aomenHttps == null) {
            synchronized (HttpUtils.class) {
                if (aomenHttps == null) {
                    aomenHttps = getBuilder(BASE_URL2).build().create(a);
                }
            }
        }
        return (T) aomenHttps;
    }
    public <T> T getSportServer(Class<T> a) {
        if (sportHttps == null) {
            synchronized (HttpUtils.class) {
                if (sportHttps == null) {
                    sportHttps = getBuilder(SPORT_URL).build().create(a);
                }
            }
        }
        return (T) sportHttps;
