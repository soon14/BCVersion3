package com.mrice.txl.appthree.ui.zoushi;
...
public class LottoTrendActivity extends AppCompatActivity implements DDTrendChart.ISelectedChangeListener {
    ...
    protected void onCreate(Bundle bundle) {
        ...
        if (position == 1) {
            toolbar.setTitle("双色球趋势分析");
        } else if (position == 2) {
            toolbar.setTitle("大乐透趋势分析");
        } else {
            toolbar.setTitle("七星彩趋势分析");
        }
...
//        });
        try {
            ...
            if (position == 1) {
                input = asset.open("ssq.xml");
            } else {
                input = asset.open("dlt.xml");
            }
            a(input);
        } catch (Exception ex) {
